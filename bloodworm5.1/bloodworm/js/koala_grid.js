/**
 * @author zjh
 * +function($){}(window.jquery)->http://suqing.iteye.com/blog/1981591
 */

+function ($) {

	"use strict";//严格模式，放在脚本文件的第一行，建议使用->http://www.xuanfengge.com/4637.html
	...
	var Grid = function(element, options){
		this.$element = $(element);//$(element)快速获取某个元素，类似于document.getElementById("element")
...
//				});
			...
			var widthRgExp= /^[1-9]\d*\.?\d*(px){0,1}$/;//正则表达式，https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Guide/Regular_Expressions#创建一个正则表达式
			var titleHtml = new Array();
			titleHtml.push('<tr>');
			if(this.options.isShowIndexCol){//复选按钮
			...
			for(var i= 0, j=columns.length; i<j; i++){
				...
				titleHtml.push('<th index="'+i+'" width="');//<th index="i" width="**px" class="sort" sortName="***" title="点击排序">列名称<div class="colResize"></div></th>
				if(width.match(widthRgExp)){
					width = width.replace('px', '');//后面参数替换前面参数
			...
			this.gridTableHeadTable.html(titleHtml.join(''));//修改.grid-table-head table,=table.html(..)的内容，将<th>..</th>加进table中，参见koloa.css
			if(totalColumnWidth > this.$element.width()){//总宽度大于所有列所需的宽度，则设总宽度等于列表总宽度
			...
			this.gridTableHeadTable.find('[data-role="selectAll"]').on('click',function(e) {//全选按钮按下
				e.stopPropagation();//阻止冒泡事件，这个事件的对象就是函数的参数e，e这个名字可以随意取，参见http://www.tuicool.com/articles/uInI32F
				var $this = $(this);//将按钮这个动作赋值给一个变量
				if($this.hasClass('checked')){//如果添加了.checked则说明按钮被按下，再按一次就讲按钮取消选中
					self.gridTableBodyTable.find('[data-role="indexCheckbox"]').each(function(){//遍历所有子按钮
					...
					self.gridTableBodyTable.find('[data-role="indexCheckbox"]').each(function(){
						$(this).addClass('checked').closest('tr').addClass('success');			//选中按钮
					});
				}
				$this.toggleClass('checked');//全选按钮切换
			});
		/*	var sorts = this.gridTableHeadTable.find('.sort');  //这里用不到排序
		...
		_initOptions: function(){//匿名函数
			...
			//修改每页显示的条数，并调整页面
  			this.pageSizeSelect.setValue(this.options.pageSize).on('change', function(){
  				self.pageSize = $(this).getValue();//每页显示多少条数据，采用物理分页方式
  				self.pageNo = Grid.DEFAULTS.pageNo;//从第一页开始显示
  				self._loadData();//重载数据？
  			});
			if(self.options.querys && self.options.querys.length>0){
				this.condition.select({ //this.searchContainer = this.$element.find('.search');
				                        // this.condition = this.searchContainer.find('[data-role="condition"]');
					title: '选择条件',
					contents: self.options.querys//查询选择条件的具体内容
				});
			}else{
		...
		_initEvents: function(){
	        ...
	        this.gridTableBody.on('scroll', function(){
				self.gridTableHead.css('left', -$(this).scrollLeft());//获得水平滚动条的水平位置,在该项目中目前用不到
			});
		...
		_loadData: function(){
			var self = this;
			var params = {};//对象
			params.pagesize = self.pageSize;//每页显示多少条数据
			params.page = self.pageNo;//页码
			for(var prop in self.searchCondition){//查询searchCondition中的每一个属性
            ...
            if(self.options.isUserLocalData){//使用本地数据
            	var start = self.pageSize * (self.pageNo-1);//10*(10-1)第10页的第90条数据
            	var end =  self.pageSize * self.pageNo-1;//10*10-1
            	self.totalRecord = self.options.localData.length;
            	self.startRecord.text(start + 1);//显示91
				self.endRecord.text(end+1);//显示100
				self.totalRecordHtml.text(self.totalRecord);//填写总记录
		...
		_initPageNo: function(){
			...
			pageHtml.push('<li data-role="firstPage"><a href="#">&laquo;</a></li>');//页码显示
			pageHtml.push('<li data-role="prev"><a href="#">&lsaquo;</a></li>');
			if((self.pageNo-1) % self.showPage == 0){ //页码是1234，或5678....
				self.pageNo != 1 && pageHtml.push('<li><a href="#">...</a></li>');//首页页码不是1的时候，前面加省略号
			...
			pagination.html(pageHtml.join('')).find('li[data-role="pageNo"]').on('click', function(){
				self.pageNo = $(this).data('value');//获取当前页数，页面更新
				self._loadData();//页面更新
			}).end().find('li[data-value="'+self.pageNo+'"]').addClass('active');//按钮颜色变化
        ...
        initSelectRowEvent: function(){
            ...
            indexCheckboxs.off('click').on('click',function(e) {
                ...
                if($this.hasClass('checked')){
                    $this.removeClass('checked').closest('tr').removeClass('success');//选择后那一行颜色变化
                }else{
                    $this.addClass('checked').closest('tr').addClass('success');
                }
                if(self.selectedRowsIndex().length == indexCheckboxs.length){//判断是否全选
                    selectAll.addClass('checked');
                }else{
                    selectAll.removeClass('checked');
                }
                self.$element.trigger('selectedRow', {checked: $this.hasClass('checked'), item:self.items[$this.attr('indexValue')]});//selectedRow选中行，{}中是额外的参数
            });
            this.gridTableBodyTable.find('tr').off('click').on('click', function(){//在每一行的任意位置点击都会执行该行选中
                var $this = $(this);
                ...
                if(self.selectedRowsIndex().length == indexCheckboxs.length){//判断如果该页所有的行都被选中，则自动选中全选按钮
                    selectAll.addClass('checked');
                }else{
                    selectAll.removeClass('checked');
                }
		...
		renderRows: function(){
			var self = this;
			if(self.options.tree && self.options.tree.column){//是 否是树型结构，好像并没有用到
			...
			for(var i= 0,j=items.length; i<j; i++){
				var item = items[i];
                self.itemsMap[item.id] = item;//itemsMap是一个对象，[]中是属性，=后是属性值。
				...
				if(this.options.isShowIndexCol){//是否显示索引列
				...
				trHtml.push('</tr>');
				trHtmls.push(trHtml.join(''));//.join()是将数组中的所有元素放入到一个字符串，元素是通过指定的分隔符进行分隔。
			}
		...
		selectedRows: function(){
			...
			this.gridTableBodyTable.find('.checked[data-role="indexCheckbox"]').each(function(){
				selectItems.push(self.items[$(this).attr('indexvalue')]);//索引序列值
			});
			return  selectItems;
		},
		/**
		...
		,_handleResult:function(self,result){
			...
			self.items = result.Rows;//返回框架中行的数目
	...
	$.fn.getGrid = function(){//$.fn是指jquery的命名空间，加上fn上的方法及属性，会对jquery实例每一个有效
