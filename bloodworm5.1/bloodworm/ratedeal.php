...
for($i=0;$i<count($array_sum);$i++)
{
    if($risk->AcuteInfections[$i]!=0 or $risk->DeterminationOfContaminatedWater[$i]!=0)
    {
        $R[$i]=3;
    }
    else if($risk->SnailIS_SchistosomaInfection[$i]!=0 or $risk->StoolRatio[$i]!=0)
    {
        $R[$i]=2;
    }
    else
...
for($i=0;$i<count($array_sum);$i++)
{
    ...
    if($risk->RiskIndex[$i]<1.5) {$risk->Color_WL[$i]=1;}
    else if($risk->RiskIndex[$i]<2.5) {$risk->Color_WL[$i]=2;}
    else if ($risk->RiskIndex[$i]<4.5) {$risk->Color_WL[$i]=3;}
    else {$risk->Color_WL[$i]=4;}

    $Task_Id=strval( $risk->Task_Id[$i]);
    $RiskIndex=floatval($risk->RiskIndex[$i]);
    $Color_WL=intval( $risk->Color_WL[$i]);
    $sql_1=mysqli_query($con,"UPDATE tb_is_sum SET RiskIndex=$RiskIndex,Color_WL=$Color_WL
