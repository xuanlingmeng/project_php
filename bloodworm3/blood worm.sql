...
CREATE TABLE `tb_e_eliminatesnail` (
  ...
  `Method` int(6) NOT NULL COMMENT '灭螺方法',
  ...
  `Money` double(10,4) NOT NULL COMMENT '工程实际支出费用',
...
CREATE TABLE `tb_e_task` (
  ...
  `Latitude_e` double(10,5) NOT NULL COMMENT '最东纬度（最小）',
  `Latitude_w` double(10,5) NOT NULL COMMENT '最西纬度（最大）',
  `Longitude_n` double(10,5) NOT NULL COMMENT '最北经度（大）',
  `Longitude_s` double(10,5) NOT NULL COMMENT '最南经度（小）',
  ...
  PRIMARY KEY (`_Id`),
  KEY `fk_WLO` (`Task_Id`),
  CONSTRAINT `fk_WLM` FOREIGN KEY (`Task_Id`) REFERENCES `tb_wlm_` (`Task_Id`),
...
-- ----------------------------
-- Table structure for tb_wlm_
-- ----------------------------
DROP TABLE IF EXISTS `tb_wlm_`;
CREATE TABLE `tb_wlm_` (
  ...
  `Order_Id` int(2) DEFAULT NULL COMMENT '坐标点顺序编号',
  `GPSstate` bit(1) DEFAULT NULL COMMENT 'GPS定位状态',
  `GPS_OnorOff` bit(1) DEFAULT NULL COMMENT 'GPS是否打开',
  PRIMARY KEY (`M_Id`),
  KEY `WL_Id` (`Task_Id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
