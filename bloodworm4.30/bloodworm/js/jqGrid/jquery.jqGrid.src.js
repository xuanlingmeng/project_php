						...
						if (!$(this).hasClass('ui-state-disabled')) {
							...
							if (sr) {
								if($.isFunction( o.editfunc ) ) {
									...
									//鼠标选中后出发的事件，此处为跳转新页面
									var b = $("#grid-table tr[aria-selected=true] td[aria-describedby=grid-table_taskid]").html();
									$($t).jqGrid("editGridRow",sr,pEdit);
									//$("#grid-table tr[aria-selected=true] td[aria-describedby=grid-table_taskid]").wrap("<a href=" + a + "></a>");
