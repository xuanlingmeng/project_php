							...
							.click(function(){
								...
								if (!$(this).hasClass('ui-state-disabled')) {
									...
									if (sr) {
										...
										//	$("#grid-table").setRowData( id, { email:"741@qq.com"});//动态修改某一行的值
										var username=[];
										var rowData = [];
										var rowid = $("#grid-table").getGridParam("selarrrow");
											for(var i=0;i<rowid.length;i++)
											{
												rowData[i] = $("#grid-table").getRowData(rowid[i]);
												username[i]=rowData[i].username;
											//	$("#grid-table").setRowData( rowid[i], { decision:"1"});
												//alert(rowData[i].email
											}
										var b=confirm("是否同意已选中的申请信息?");
											if(b==true)
											{
												for(var i=0;i<rowid.length;i++)
												{
												...
												window.location.href='personrequestspass.php?str=' + username;
											}
											else
											{
												alert("请重新选择或返回其他界面！");
										...
										//confirm("是否拒绝所有注册申请?");
										var a=confirm("是否拒绝所有注册申请?");;
										if(a==true)
										{
											window.location.href='personrequestspass.php?str=' + 0;
										}
										else
										{
											alert("请选择同意注册人员的信息！");
